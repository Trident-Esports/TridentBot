...
module.exports = class VillainsCommand extends Command {
    ...
    /**
     * @type {TextChannel} Channel to send embeds to
     * @private
     */
    #textChannel;   // Private: Channel to send VillainsEmbed to
    /**
    ...
    async processArgs(message, args, flags = { user: "default", target: "invalid", bot: "invalid", search: "valid" }) {
        ...
        let search = (args && (args.length > 0) && (!(mention)) && message?.guild?.members) ? await message.guild.members.fetch({ query: args.join(" "), limit: 1 }) : undefined
        ...
        if (loaded) {
            ...
            if (this.flags.user == "invalid" && user?.id && loaded.id == user.id) {
    ...
    async run(message, args, cmd) {
        // @ts-ignore
        const passVals = args?.parser?.parserOutput?.ordered.map(a => a.value) || args
        // @ts-ignore
        const passRaws = args?.parser?.parserOutput?.ordered.map(a => a.raw) || args
        // Process arguments
        await this.processArgs(message, passVals, this.flags)

        // Build the thing
        await this.build(message, passVals, cmd)
